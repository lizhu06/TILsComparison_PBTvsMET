...
immune <- immune_res_aveDup$estimate[,"ImmuneScore", drop=FALSE]
...
pdf(file="Results_v2/boxplot_raw_immune_primary_mets.pdf", width=3, height=2.5)
plot(p)
dev.off()
...
pdf(file="Results_v2/boxplot_raw_immune_change.pdf", width=2, height=2.5)
plot(p_delta)
dev.off()
...
for(i in 1:4){
  ...
  pdf(file=paste0("Results_v2/boxplot_raw_immune_primary_mets_", site_vec[i], ".pdf"), 
    width=3, height=2.5)
  plot(p)
  dev.off()
...
for(i in 1:4){
  ...
  pdf(file=paste0("Results_v2/boxplot_raw_immune_primary_mets_", HR_HER2_name_vec_fileName[i], ".pdf"), 
    width=3, height=2.5)
  plot(p)
  dev.off()
  fit <- wilcox.test(data_plot[,3], 
