...
immune[[1]] <- immune[[1]][,"ImmuneScore", drop=FALSE]
immune <- immune[-5]
		...
		for(c in 1:length(choice_vec)){
			if(choice_vec[c]=="primary"){
				...
				mets_id <- sample_pair_info[match(primary_id,
						sample_pair_info[,"primary_id"]), "mets_id"]
				score <- data[mets_id, , drop=FALSE] - data[primary_id, , drop=FALSE]
			}
		    ...
		    value <- factor(value_factor, levels=c("<=median", ">median"))
		    surv_obj <- surv_obj[!miss_index, ]
		    
		    # log-rank test
		    fit <- survdiff(surv_obj ~ value)
		    p1 <- round(1 - pchisq(fit$chisq, length(levels(value))-1), digits=4)
		    if(p1==0){
		      p1 <- "<0.0001"
		    }
		    pval_mat[i, c] <- p1
		    ...
		    survdata <- data.frame(time=surv_obj[,1], status=surv_obj[, 2], 
		                           group=value)
		    if(plot_km){
